...
public class TestTotalTaxes {
	...
	@Test
	public void test_GIVEN_item_WHEN_defult_percentage_THEN_calculate_total_amount() {
		setUp();
		
		ArrayList<Item> items = retailServices.getAllItem();
		double totalPrice = 20*2+20*2*1.2+15*1*1.2;
		double totalMRP =0.0;
		for (Item item : items) {
			totalMRP +=retailServices.getMaxRetailPrice(item);
		}
		assertEquals(totalPrice, totalMRP);
	}
